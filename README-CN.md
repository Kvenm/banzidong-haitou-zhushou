...
更完整的字段说明、环境变量与排错见 **[docs/使用指南.md](docs/使用指南.md)**（其中 **§2.1** 为前端使用详解）。
...
2. 填写仓库名（例如 `haitou-assistant-next`），选 Public / Private。  
3. **不要**勾选「Add a README」等自动生成文件（若你本地已有完整项目，避免首次推送冲突）。  
4. 创建后页面会显示空仓库的推送提示，记下 HTTPS 或 SSH 地址，例如：  
   `https://github.com/<你的用户名>/<仓库名>.git`

### 2. 本地推送（首次）

在 **`haitou-assistant-next` 根目录**执行（按你实际路径 `cd`）：
